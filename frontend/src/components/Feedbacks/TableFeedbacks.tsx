...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/feedbacks/feedbacksSlice';
...
const TableSampleFeedbacks = ({
  ...
  useEffect(() => {
    if (feedbacksNotify.showNotification) {
      notify(
        feedbacksNotify.typeNotification,
        feedbacksNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/feedbacks/feedbacks-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {feedbacks && Array.isArray(feedbacks) && !showGrid && (
        <ListFeedbacks
          feedbacks={feedbacks}
          loading={loading}

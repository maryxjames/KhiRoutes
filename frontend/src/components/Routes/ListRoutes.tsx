...
const ListRoutes = ({
  ...
  const hasUpdatePermission = hasPermission(currentUser, 'UPDATE_ROUTES');
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div className={'flex-1 px-3'}>
                    <p className={'text-xs   text-gray-500 '}>StartPoint</p>
                    <p className={'line-clamp-2'}>{item.start_point}</p>
                  </div>

                  <div className={'flex-1 px-3'}>
                    <p className={'text-xs   text-gray-500 '}>EndPoint</p>
                    <p className={'line-clamp-2'}>{item.end_point}</p>
                  </div>
                </div>
                <ListActionsPopover
                  onDelete={onDelete}
                  ...
                  onEdit={onEdit}
                  itemId={item.id}
                  pathEdit={`/routes/routes-edit/?id=${item.id}`}
                  pathView={`/routes/routes-view/?id=${item.id}`}
